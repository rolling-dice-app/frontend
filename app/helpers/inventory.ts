import type { CurrencyAmount, InventoryItemDTO } from '@rolling-dice-app/core'
import { CARRY_WEIGHT_PER_STR, COINS_PER_LB } from '~/constants/inventory'

type WeighedItem = Pick<InventoryItemDTO, 'weight' | 'quantity'>

/** 計算物品列表的總重量（每件 weight × quantity 加總） */
export function calculateItemsWeight(items: ReadonlyArray<WeighedItem>): number {
  return items.reduce((sum, item) => sum + item.weight * item.quantity, 0)
}

/** 計算金幣總重量（所有幣種枚數合計 / COINS_PER_LB） */
export function calculateCurrencyWeight(currency: CurrencyAmount): number {
...
export function calculateBackpackLoad(
  items: ReadonlyArray<WeighedItem>,
